...
Fully implements Document 11 / ragbot.md Architecture:
...
- Hybrid Search (Dense + Keyword BM25) & Cross-Encoder Reranking
- Context Compression & Citation Generation
"""
...
class PermissionResolver:
    ...
    def filter_accessible_chunks(
        ...
        for chunk in chunks:
            ...
            # Rule 2: GLOBAL visibility accessible to all workspace users
            if meta.visibility in ("GLOBAL", "SYSTEM"):
                allowed_chunks.append(chunk)
                continue

            # Rule 3: PRIVATE visibility accessible ONLY to owner
            if meta.visibility == "PRIVATE" and meta.owner_id == user_id:
                allowed_chunks.append(chunk)
                continue

            # Rule 4: TEAM visibility accessible ONLY if user belongs to department
...
class EnterpriseRAGEngine:
    ...
    async def query_knowledge_base(
        ...
        # 2. Permission Resolver Filtering BEFORE Similarity Search
        ...
        # 3. Hybrid Search & Reranking (Simulated Top 6 Context Compression)
