...
export default function FinancePage() {
  ...
  const handleAskCFO = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast.success('Finance Agent analyzed your request and updated forecasts!', {
        icon: '💰',
      });
      setPrompt('');
    }, 1200);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
        ...
        <button
          onClick={() => {
            toast.info('CEO Planner requested a full audit from Finance Agent...');
          }}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white hover:bg-white/10 text-xs font-semibold transition-all self-start md:self-auto"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2 Cols: Cash Flow Projection & Breakdown */}
          ...
          <GlowCard>
            ...
            <div className="mt-4 flex items-center justify-between text-xs text-slate-400">
              <div className="flex items-center gap-4">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-rose-500/50" /> Burn
                </span>
              </div>
              <span>Next Milestone: Series A Target ($50k MRR)</span>
            </div>
          </GlowCard>

          {/* Expense Categories */}
          <GlowCard>
            <h3 className="text-base font-bold text-white mb-4">Monthly Expense Distribution</h3>
            <div className="space-y-3">
              {[
                ...
                {
                  name: 'Cloud Infrastructure & GPU',
                  amount: '$6,500',
                  pct: 23,
                  color: 'bg-purple-500',
                },
                { name: 'Marketing & Ads', amount: '$2,500', pct: 9, color: 'bg-emerald-500' },
                { name: 'Legal & Software SaaS', amount: '$1,500', pct: 5, color: 'bg-amber-500' },
              ].map((item) => (
                <div key={item.name} className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-300 font-medium">{item.name}</span>
                    <span className="text-white font-semibold">
                      {item.amount} <span className="text-slate-500">({item.pct}%)</span>
                    </span>
                  </div>
        ...
        {/* Right 1 Col: AI CFO Command & Recommendations */}
        <div className="space-y-6">
          ...
          {/* CFO Recommendations */}
          <GlowCard>
            <h3 className="text-base font-bold text-white mb-4">CFO Recommendations</h3>
            <div className="space-y-3">
              <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs space-y-1">
                <div className="flex items-center gap-1.5 text-amber-400 font-semibold">
                  <AlertCircle size={14} />
                  Cloud Host Reserved Instances
                </div>
                <p className="text-slate-300 leading-relaxed">
                  Switching GPU nodes to 1-year reserved contracts will cut server burn by
                  $1,800/mo.
                </p>
              </div>
              ...
              <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs space-y-1">
                ...
                <p className="text-slate-300 leading-relaxed">
                  With current growth rate, profitability target is reachable in 10 months.
                </p>
              </div>
            </div>
          </GlowCard>
