    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold transition-all shadow-lg shadow-purple-500/20 self-start md:self-auto"
        >
          <Plus size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <GlowCard glowColor="rgba(168, 85, 247, 0.15)">
          <div className="text-xs text-slate-400 font-medium mb-2">Open Roles</div>
          <div className="text-3xl font-extrabold text-white">4 Active</div>
          <div className="text-xs text-purple-300 mt-2">AI Engineer, Full Stack, Product</div>
        </GlowCard>

        <GlowCard glowColor="rgba(99, 102, 241, 0.15)">
          <div className="text-xs text-slate-400 font-medium mb-2">Total Applicants</div>
          <div className="text-3xl font-extrabold text-white">128</div>
          <div className="text-xs text-indigo-300 mt-2">Parsed by Talent AI</div>
        </GlowCard>

        <GlowCard glowColor="rgba(16, 185, 129, 0.15)">
          <div className="text-xs text-slate-400 font-medium mb-2">Offers Pending</div>
          <div className="text-3xl font-extrabold text-white">1 Ready</div>
          <div className="text-xs text-emerald-300 mt-2">Elena Rostova ($165k + equity)</div>
        </GlowCard>

        <GlowCard glowColor="rgba(245, 158, 11, 0.15)">
          <div className="text-xs text-slate-400 font-medium mb-2">Avg Time to Hire</div>
          <div className="text-3xl font-extrabold text-white">12 Days</div>
          <div className="text-xs text-amber-300 mt-2">60% faster than industry avg</div>
        </GlowCard>
      </div>

      {/* Pipeline Board & Selected Candidate Detail */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-1.5 bg-white/5 border border-white/10 rounded-xl text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500"
              />
            </div>
          </div>
        ...
        {/* Candidate Detail or Action Box */}
        <div>
          <GlowCard glowColor="rgba(168, 85, 247, 0.2)">
            ...
            {selectedCandidate ? (
              <div className="space-y-4 text-xs">
                <div className="p-3.5 rounded-xl bg-white/5 border border-white/10 space-y-2">
                  ...
                  <p className="text-slate-300 leading-relaxed">
                    Strong background in distributed LLM serving and WebGL rendering.
                    Recommendation: Proceed to offer stage.
                  </p>
                </div>
                ...
                <button
                  onClick={() => {
                    toast.success(
                      `Drafted offer letter for ${selectedCandidate.name}! Sent to Founder Approval.`,
                    );
                  }}
                  className="w-full py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold text-xs rounded-xl shadow-lg hover:from-purple-500 hover:to-indigo-500 transition-all"
                >
              ...
              <div className="text-center py-8 space-y-3 text-slate-400 text-xs">
                <Users size={32} className="mx-auto text-purple-500/50" />
                <p>
                  Select a candidate from the pipeline to view AI scoring, resume audit, and offer
                  generation tools.
                </p>
              </div>
            )}
